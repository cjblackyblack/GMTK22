...
using UnityEngine;
[CreateAssetMenu(menuName = "SmartState/Wizard/AttackState")]
public class WizardAttackState : SmartState
{
    [SerializeField]
    ...
    protected void CreateProjectiles(SmartObject smartObject)
    {
        for (int i = 0; i < projectiles.Length; i++)
        {
            if (smartObject.currentTime == projectileTime[i])
            {
                ...
                Vector3 pos1 = smartObject.tform.position;
                Vector3 pos2 = PlayerManager.current.Party[UnityEngine.Random.Range(0, PlayerManager.current.Party.Length)].tform.position;
                Vector3 iDir = (pos2-pos1).normalized;
                projectile.inputDir = new Vector2(iDir.x,iDir.z);
