...
using UnityEngine;
[CreateAssetMenu(menuName = "SmartState/Ninja/AttackState")]
public class NinjaAttackState : SmartState
{
    [SerializeField]
    ...
    protected void CreateProjectiles(SmartObject smartObject)
    {
        for (int i = 0; i < projectiles.Length; i++)
        {
            if (smartObject.currentTime == projectileTime[i])
            {
                ...
                projectile.inputDir = smartObject.stateMachine.pointMem.NinjaShoot;
